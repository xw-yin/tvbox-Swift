...
3. **解析链路能力不同**
   ...
   - Swift 版已接入完整播放解析链路（`ParseChainService`）：非直链地址自动按序尝试解析接口还原真实播放地址——`type=1` JSON 接口兼容 JSON/JSONP/纯文本返回，`type=0` 嗅探接口经由 `WebSniffService` 用无头 WKWebView 真实加载页面（响应 MIME / 跳转 / JS 轮询三路捕获）；并按历史延迟优选接口。规则引擎与相关过滤策略尚未接入。
4. **播放器能力不同**
...
6. **直播高级能力不同**
   ...
   - Swift 版当前已支持基础直播分组与线路切换，并已接入远程 EPG（`EpgService`）：按直播配置中的 `epg` URL 模板（`{name}` 占位符）拉取节目单，兼容 diyp 风格 JSON 与裸数组，播放器信息栏显示当前节目与下个节目。
7. **筛选与检索形态不同**
   - 安卓版具备分类筛选、快速搜索、复选搜索等界面与交互。
   - Swift 版已支持分类筛选栏（年份/地区/类型等，点选生效、重复点击取消），其余为基础搜索与分类浏览。
8. **远程控制与推送不同**
